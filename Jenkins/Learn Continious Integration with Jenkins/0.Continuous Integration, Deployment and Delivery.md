...
* Easy installation: Jenkins is a self-contained Java program. This means it can be run on most operating systems such as Winodws, MAC and Linux. 
...
* Extensible: Jenkins is also extremely extensible. Jenkins can be extended via its plugins architecture and provides many oppurtunities. 
* Distribution: Jenkins is really easy to have available across your entire network, in other words, it can easily distribute work acorss multiple machines, helping in faster builds, tests and deployments across multiple platforms. 
...
**Code Commit**: When you are doing developement, you start to first write the code on your machine. When you are using a Jenkins pipeline, you then commit your code.
...
**Release**: If everything passes the tests, everything will get placed in the stage and release-ready enviroment within Jenkins.

**Deploy/Deliver**: from the stage and release-ready enviroment, everything is placed in the Deploy/Deliver enviroment. Jenkins is the tool which will help you with your server environment to be able to deploy your code to the production environment
